...
Устройство:
- `sorm.From[E](db)` — тот же иммутабельный билдер, что `Query[E]`, плюс `GroupBy(exprs ...GroupExpr[E])`, `Having(preds ...Pred[E])`, `Distinct()`, `DistinctOn(...)`.
- Агрегатные выражения — свободные функции (им нужны свои type params, методам билдера — нельзя): `sorm.CountAll()`, `sorm.Count(col)`, `sorm.Sum(User.Age)`, `sorm.Avg/Min/Max(...)`. Возвращают `AggExpr[E, V]` с методами-сравнениями (`.Gt(10)` → `Pred[E]` для HAVING) — типобезопасно и на типе значения, и на сущности.
- `sorm.Project[R](builder, exprs...)` — свободная функция с независимым параметром `R`. Маппинг колонок результата на поля `R` — по тегу/имени, план сканирования строится **один раз на тип `R` через рефлексию и кэшируется** (осознанное исключение из «zero reflection»: R — произвольная пользовательская структура; стоимость — один раз на тип, как регистрация модели в Bun).
- HAVING-предикаты валидируются: агрегатный `Pred` вне `Having` → ошибка построения.
...
- `LeftJoin(rel)` / `InnerJoin(rel)` по связи — оба типа известны, полная типобезопасность.
- Произвольный `ON` — через свободную функцию `sorm.ColEq(a Col[E1,V], b Col[E2,V])` (свободные функции могут вводить два параметра сущностей; типы **значений** обязаны совпадать — сравнение int-колонки со string-колонкой не скомпилируется). Принадлежность колонок таблицам из FROM/JOIN-набора проверяется при построении запроса с внятной ошибкой — это рантайм-валидация, как у Jet: на кросс-табличном уровне энтити-параметризация ослабляется осознанно.
- `RIGHT JOIN` — поддерживается как `RightJoinOn` для полноты, но в документации честно рекомендуем переписывать в LEFT (читаемость).
- После JOIN'ов `Where` принимает предикаты по любой присоединённой сущности (тот же ослабленный режим с валидацией принадлежности).
