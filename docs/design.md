...
Всё в пакете `sorm/migrate`; движок диффа — `ariga.io/atlas` как Go-зависимость (v0.32, пин ради Go 1.25). **Внешний atlas CLI не нужен, docker сам не поднимается**: если операции нужна БД — пользователь поднимает её сам и передаёт DSN. Рантайм sorm от Atlas не зависит — зависимость линкуется только при импорте `sorm/migrate`.

1. **Декларативно из кода** — `migrate.Apply(ctx, sdb, "postgres")`: инспекция БД (ограничена таблицами sorm — чужое не трогается) → дифф против зарегистрированных моделей (`sorm gen` эмитит `TableDef` в sormgen) → применение. `migrate.Plan` — dry-run SQL. Аналог безопасного AutoMigrate: дрейф детектируется и чинится.
2. **Версионные файлы из кода** — `migrate.Diff(ctx, devDB, dialect, dir, name)`: существующие миграции проигрываются (replay) на пустой scratch-БД пользователя (для SQLite — `:memory:`, поднимать нечего), состояние диффуется против моделей, разница пишется файлом `<utc-timestamp>_<name>.sql`. `migrate.Up(ctx, db, dialect, dir)` применяет неприменённые файлы по порядку (учёт — таблица `sorm_migrations`; PG/SQLite — файл в транзакции, MySQL — постейтментно, DDL там неявно коммитится); `migrate.Pending` — список неприменённых.
3. **CLI-обёртки над тем же кодом** — `sorm migrate diff -dev-dsn <dsn> <name> [models]` (модели читаются парсером, sormgen не нужен) и `sorm migrate up -dsn <dsn>`.

Интеграционные тесты создают таблицы сгенерированной схемой — DDL-генератор проверен рантаймом на всех трёх диалектах; версионный цикл (diff → up → идемпотентность → эволюция схемы → pending) покрыт тестами на SQLite.
