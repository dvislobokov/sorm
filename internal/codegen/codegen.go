...
func Generate(s *parse.Schema) (map[string][]byte, error) {
	...
	ctxSrc, err := genContext(s)
	if err != nil {
		return nil, err
	}
	out["context.go"] = ctxSrc
...
// genContext emits the EF-style Context: a Session plus a typed Set per
// entity. Set field names come from the table name (users → Users,
// api_keys → ApiKeys) — plural, like DbSet properties in EF Core.
func genContext(s *parse.Schema) ([]byte, error) {
	g := &gen{}
	g.pf("// Code generated by sorm gen. DO NOT EDIT.\n\npackage sormgen\n\nimport (\n\t\"context\"\n\n\t\"github.com/dvislobokov/sorm\"\n\n\tmodels %q\n)\n\n", s.PkgPath)

	g.pf("// Context is the unit-of-work facade: a sorm.Session plus a typed set\n")
	g.pf("// per entity. Queries via sets are tracked (identity map + snapshots);\n")
	g.pf("// mutate entities with plain Go code and call SaveChanges.\n")
	g.pf("// Not thread-safe — create one per request or transaction.\n")
	g.pf("type Context struct {\n\t*sorm.Session\n\n")
	for _, e := range s.Entities {
		g.pf("\t%s sorm.Set[models.%s]\n", exportCamel(e.Table), e.Name)
	}
	g.pf("}\n\n")

	g.pf("// NewContext starts a unit of work over db (a pool or an open transaction).\n")
	g.pf("func NewContext(db sorm.DB) *Context {\n\ts := sorm.NewSession(db)\n\treturn &Context{\n\t\tSession: s,\n")
	for _, e := range s.Entities {
		g.pf("\t\t%s: sorm.NewSet[models.%s](s),\n", exportCamel(e.Table), e.Name)
	}
	g.pf("\t}\n}\n\n")

	g.pf("// RunInTx runs fn in a transaction: commit on nil, rollback on error,\n")
	g.pf("// retries on transient failures (serialization, deadlock). Each attempt\n")
	g.pf("// gets a FRESH child context bound to the transaction — work only\n")
	g.pf("// through it inside fn; the parent context's state is not visible and\n")
	g.pf("// must not be mixed in. SaveChanges on the child flushes into this\n")
	g.pf("// transaction without opening a nested one.\n")
	g.pf("func (c *Context) RunInTx(ctx context.Context, fn func(txc *Context) error) error {\n")
	g.pf("\treturn sorm.RunInTx(ctx, c.Session.DB(), func(tx sorm.Tx) error {\n\t\treturn fn(NewContext(tx))\n\t})\n}\n")

	formatted, err := format.Source([]byte(g.String()))
	if err != nil {
		return nil, fmt.Errorf("context: generated code does not parse: %w\n%s", err, g.String())
	}
	return formatted, nil
}

// exportCamel turns a snake_case table name into an exported Go name:
// users → Users, api_keys → ApiKeys.
func exportCamel(snake string) string {
